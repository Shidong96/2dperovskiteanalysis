...
class PlotPDOS():
	...
	for j in 2,4:
		for i in range(0,3):
  		      plt.plot(energy, elementPDOS[j][i]/volume, linestyle[j-1],linewidth=3, color=c[i], label=elements[j]+"-"+obrt[i])
	...
	plt.legend(loc='upper center', bbox_to_anchor=(0.75,0.95),ncol=3,fontsize = 32, frameon = False)
