...
class AbstractPDOS(object):
	...
	with open(self.path+'/DOSCAR') as f:
		...
		temp_dos_down = []
			...
			up.append(self.get_pdos(temp, o_index))
		    temp_dos_up.append(up)
 			
            	    if ispin == 2:
		        for o in orbitals: 
			    o_index = np.array(orbital[o])
			    down.append(self.get_pdos(temp, o_index+1))
	...
	if len(sys.argv) == 2 and ( str(sys.argv[1]) == 'T' or str(sys.argv[1]) == 't'):
        ...
        energy, ispin, dos = self.getDOS()

        a, b, c, volume = self.getLatticeVector()
