...
fileanglexy = open('fileanglexy','w')
...
while 1>0:
    ...
    averageanglexy = float(sum(anglexy))/len(anglexy)
    averagelength = float(sum(length))/len(length)
    for i in xrange(0,len(anglez)):
        anglez[i] = str(anglez[i])
        anglexy[i] = str(anglexy[i])
        length[i] = str(length[i])
    fileanglez.write('     '+time+'     '+str(averageanglez)+' '+' '.join(anglez)+'\n')
    fileanglexy.write('     '+time+'     '+str(averageanglexy)+' '+' '.join(anglexy)+'\n')
    filelength.write('     '+time+'     '+str(averagelength)+' '+' '.join(length)+'\n')
    os.popen('rm tempposcar')
